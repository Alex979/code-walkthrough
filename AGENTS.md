...
- Keep the Node.js 22+ runtime free of third-party dependencies. Ship prebuilt Node ESM tools so users need no Bun, package installation, TypeScript runner, or build step. Maintain source in TypeScript and use Bun only for development. Resolve package resources relative to the skill root so symlinked and copied installations work from other working directories.
...
Run Prettier on maintained files with `bun run format`; `bun run format:check` enforces the shared style. Generated viewer assets, Node bundles, and captured example data are excluded. Prettier is a development dependency only; the installable skill remains free of third-party runtime dependencies.
...
Install Bun 1.3+ for development and Node.js 22+ for checks of the distributed tools. Run from the repository root:

```sh
bun run check
```

For capture, schema, or validator changes, use a synthetic fixture to run capture followed by `node skills/code-walkthrough/scripts/validate.mjs ARTIFACT`. Check both a complete valid lesson and relevant rejection cases: illegal paths, invalid source locations, or a final state that differs from head. For scope changes, exercise the affected scope and confirm its endpoints. Rebuild bundled tools and assets after runtime or viewer changes. Verify copied-package capture, validation, and serving with Node, without relying on Bun at runtime.
