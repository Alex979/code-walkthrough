...
import { configureLesson, fileExists, fileStatus, sourceAtStep, resolveFocus, diffLines, parseLocation, makeLocation } from "../viewer/src/model";
import type { FileInfo, Step } from "../viewer/src/types";
...
const steps: Step[] = [
  ...
  { id: "shape", title: "Add the shape", paragraphs: [["Define the new class."]], file: added.path, changes: { [added.path]: { text: "class New {}\n" } } },
  { id: "finish", title: "Use it", paragraphs: [["Replace the old implementation."]], file: added.path, changes: { [added.path]: { use: "head" }, [removed.path]: null } },
];
...
test("focus and deep links preserve an explicit selection and version", () => {
  ...
  expect(parseLocation(makeLocation(1, added.path, "head", [2, 5], "diff")))
    .toEqual({ step: 1, path: added.path, version: "head", focus: [2, 5], mode: "diff" });
});
test("diff preserves both inputs across insertions and removals", () => {
  const before = "a\nb\nc\n", after = "a\nx\nc\nd\n";
  const rows = diffLines(before, after);
  expect(rows.filter(row => row.kind !== "add").map(row => row.text).join("\n") + "\n").toBe(before);
  expect(rows.filter(row => row.kind !== "remove").map(row => row.text).join("\n") + "\n").toBe(after);
});
