...
Use the writing guide's [connected chapter approach](writing.md#make-a-chapter-advance-a-concrete-interaction) for the opening, continuing example, and ending. These are authored paragraphs within ordinary steps, not additional schema fields. Reorder or split construction steps when that improves the teaching sequence, then recheck cumulative source locations and all later states; chapter grouping alone does not change the build.
...
When consecutive steps edit the same nearby source, the overview retains the preceding excerpt as context and highlights only the current edits. Small newly created files can continue this way too; large creations retain a bounded window near the next edit rather than their entire contents. This context is derived from lesson order, including for direct step links; it does not require an authored continuation field. Moving forward or backward through that excerpt preserves a surviving source-line anchor, then smoothly scrolls to reveal the destination step's edits, with a little context below when space permits. Edits taller than the viewport reveal their beginning. Back temporarily reserves space while the code shrinks so the browser cannot abruptly clamp the scroll position. Reduced-motion preferences skip the animation. Distant edits and changes in other files keep their own compact regions. Continue to supply precise pointers where the explanation needs them; automatic context is not a language-aware method boundary detector.
