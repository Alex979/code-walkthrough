...
7. Run `node scripts/validate.mjs ARTIFACT --presentation` from the skill root. Fix failures until every changed file reaches its exact captured head state and its text comparisons can be prepared. Review any reported coarse comparisons: all source lines remain available, but a broad replacement block may need clearer pointers or smaller teaching edits. Then complete the writing guide's [learner read-through](references/writing.md#read-it-back-as-a-learner): follow the build without links, match explanatory claims to code, and edit the prose for concrete meaning. Revise the lesson from that pass; these checks do not establish visual layout, teaching quality, or factual accuracy.
