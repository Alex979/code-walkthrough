...
4. Read [the authoring contract](references/authoring.md) and [the writing guide](references/writing.md). The [synthetic example](examples/greeting/lesson.json) demonstrates incremental edits, precise code pointers, inline formatting, and optional references. Treat `scripts/types.ts` as the exact schema. Plan each construction step's concrete edit, the reason for it, and the code that the viewer will show automatically before writing `ARTIFACT/lesson.json`. For a long lesson, add `chapters` that group meaningful build phases and name their starting step IDs. Plan each phase's starting capability, missing behavior, and ending capability, then [carry a concrete interaction through its important boundaries](references/writing.md#make-a-chapter-advance-a-concrete-interaction). Preserve one cumulative sequence without extra divider steps. Short lessons can omit chapters.
...
7. Run `node scripts/validate.mjs ARTIFACT --presentation` from the skill root. Fix failures until every changed file reaches its exact captured head state and its text comparisons can be prepared. Review any reported coarse comparisons: all source lines remain available, but a broad replacement block may need clearer pointers or smaller teaching edits.
8. Complete the [bounded review and correction stage](references/review.md) on the assembled lesson before delivery. Use an independent reviewer when delegation is available and authorized; otherwise perform a separate author review and record that limitation. Review actual cumulative edits, assembled prose, and source pointers with `scripts/review.mjs`, using the writing guide's learner criteria. Resolve concrete findings, revalidate, and recheck affected steps. Structural validation alone is not a teaching review.
