...
7. Run `node scripts/validate.mjs ARTIFACT --presentation` from the skill root. Fix failures until every changed file reaches its exact captured head state and its text comparisons can be prepared. Review any reported coarse comparisons: all source lines remain available, but a broad replacement block may need clearer pointers or smaller teaching edits. Run `node scripts/review.mjs ARTIFACT` and use `--step ID` to inspect actual cumulative edits alongside their explanation and source pointers. Complete the writing guide's [learner read-through](references/writing.md#read-it-back-as-a-learner) against that source, not just flattened prose. Revise overloaded edits, missing explanations, and mismatched pointers before delivery. The report supplies evidence for review, not a teaching-quality score.
